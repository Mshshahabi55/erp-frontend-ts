import { Typography, Chip, IconButton } from '@mui/material';
import { Visibility, Edit, Delete } from '@mui/icons-material';
import { DataTable, Column } from '@/shared/components';
import { currencyFormatter, dateFormatter } from '@/shared/utils';
import { getPurchaseStatusColor, getPurchaseStatusLabel } from '../../utils/purchaseStatusStyles';
import type { Purchase, PurchaseSortField, PurchaseStatus } from '../../types/purchase.types';

interface PurchaseTableProps {
  purchases: Purchase[];
  supplierNameById: Map<string, string>;
  isLoading: boolean;
  error: Error | null;
  totalCount: number;
  page: number;
  pageSize: number;
  onPageChange: (page: number) => void;
  onPageSizeChange: (pageSize: number) => void;
  onRetry: () => void;
  sortBy?: PurchaseSortField;
  sortDirection?: 'asc' | 'desc';
  onSortChange: (field: PurchaseSortField) => void;
  onView: (purchase: Purchase) => void;
  onEdit: (purchase: Purchase) => void;
  onDelete: (purchase: Purchase) => void;
  isDeleting: boolean;
}

export const PurchaseTable = ({
  purchases,
  supplierNameById,
  isLoading,
  error,
  totalCount,
  page,
  pageSize,
  onPageChange,
  onPageSizeChange,
  onRetry,
  sortBy,
  sortDirection,
  onSortChange,
  onView,
  onEdit,
  onDelete,
  isDeleting,
}: PurchaseTableProps) => {
  const columns: Column<Purchase>[] = [
    {
      key: 'purchaseNumber',
      label: 'PO Number',
      sortable: true,
      render: (value: string) => <Typography sx={{ fontWeight: 500 }}>{value}</Typography>,
    },
    {
      key: 'supplierId',
      label: 'Supplier',
      render: (value: string) => supplierNameById.get(value) ?? 'Unknown supplier',
    },
    {
      key: 'totalAmount',
      label: 'Amount',
      sortable: true,
      align: 'right',
      render: (value: number) => currencyFormatter.usd(value),
    },
    {
      key: 'purchaseDate',
      label: 'Date',
      sortable: true,
      render: (value: string) => dateFormatter.short(value),
    },
    {
      key: 'status',
      label: 'Status',
      sortable: true,
      render: (value: PurchaseStatus) => <Chip label={getPurchaseStatusLabel(value)} color={getPurchaseStatusColor(value)} size="small" />,
    },
    {
      key: 'id',
      label: 'Actions',
      align: 'right',
      render: (_, row) => (
        <>
          <IconButton size="small" onClick={() => onView(row)} title="View" aria-label={`View ${row.purchaseNumber}`}>
            <Visibility />
          </IconButton>
          <IconButton
            size="small"
            color="primary"
            onClick={() => onEdit(row)}
            title="Edit"
            aria-label={`Edit ${row.purchaseNumber}`}
          >
            <Edit />
          </IconButton>
          <IconButton
            size="small"
            color="error"
            onClick={() => onDelete(row)}
            disabled={isDeleting}
            title="Delete"
            aria-label={`Delete ${row.purchaseNumber}`}
          >
            <Delete />
          </IconButton>
        </>
      ),
    },
  ];

  return (
    <DataTable
      columns={columns}
      data={purchases}
      loading={isLoading}
      error={error}
      totalCount={totalCount}
      page={page}
      pageSize={pageSize}
      onPageChange={onPageChange}
      onPageSizeChange={onPageSizeChange}
      onRetry={onRetry}
      emptyMessage="No purchases found. Create your first purchase!"
      sortBy={sortBy}
      sortDirection={sortDirection}
      onSortChange={(field) => onSortChange(field as PurchaseSortField)}
    />
  );
};
